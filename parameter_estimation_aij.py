...
class parameter_estimation_aij:
    ...
    def custom_gradient_Aij(self, Aij, *args):
            ...
            for t in range(0, self.I):
                ...
                if(len(Aij.shape) < 2):
                    # print("Error surfaced!!!!!!!!!!!!!!!!!!")
                    # pdb.set_trace()
                    Aij = np.reshape(Aij, (1, Aij.shape[0]))

                # Aij_new = minimize(self.func_Aij, x0=Aij[0, :], jac=self.grad_Aij, args=(args), options={'maxiter': 1, 'eps': 3.0}, bounds=bnds, constraints=cons).x
                Aij_new = minimize(self.func_Aij, x0=Aij[0, :], jac=self.grad_Aij, args=(args), options={'maxiter': 50, 'eps': 3.0}, bounds=bnds).x
                ...
                print "Aij_new", Aij_new

                Aij = Aij_new
                shape = int(np.sqrt(len(Aij_new)))
                Aij = np.reshape(Aij, (shape,shape))
            ...
            pickle.dump(Aij, open("Aij"+str(self.U)+".p", "wb"))
            return Aij_new
    ...
    def optimize(self, dict):
        ...
        x = self.custom_gradient_Aij(Aij, args)
        dict['Aij'] = x
